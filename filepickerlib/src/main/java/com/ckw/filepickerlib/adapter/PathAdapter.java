...
import com.ckw.filepickerlib.utils.Constant;
...
public class PathAdapter extends RecyclerView.Adapter<PathAdapter.PathViewHolder> {
    ...
    public void onBindViewHolder(final PathViewHolder holder, final int position) {
        ...
        if (file.isFile()) {
            ...
            holder.tvDetail.setText(FileUtils.getNormalTime(file.lastModified()) + " / "+mContext.getString(R.string.lfile_FileSize) + " " + FileUtils.getReadableFileSize(file.length()));
